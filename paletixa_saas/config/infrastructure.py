...
def _is_development_context():
	return frappe.conf.get("developer_mode") in (1, True, "1") or bool(
...
def get_bench_path():
	...
	frappe.throw(
		frappe._(
			"No se pudo resolver la ruta del bench. Configurá bench_path en site_config.json o BENCH_PATH."
		),
...
def resolve_platform_master_sites():
	configured_sites = _normalize_items(
		frappe.conf.get("platform_master_sites") or os.environ.get("PLATFORM_MASTER_SITES")
	)
	if configured_sites:
...
def get_reserved_subdomains():
	...
	reserved_subdomains.update(
		_normalize_items(
			frappe.conf.get("platform_reserved_subdomains") or os.environ.get("PLATFORM_RESERVED_SUBDOMAINS")
		)
	)

...
def run_tests():
    print("🚀 Iniciando pruebas unitarias para el flujo de Venta Mayorista, Portal de Auto-Servicio y Reservas de Eventos...")
    ...
    test_item = items[0]
    print(f"   Artículo de prueba: '{test_item['name']}' | Precio Menudeo: ${test_item['retail_price']} | Precio Mayoreo: {test_item['wholesale_price']}")
    ...
    items_to_buy = [
        {
            "item_code": test_item_code,
            "qty": 10.0,
            "rate": test_item['wholesale_price'] if test_item['wholesale_price'] else test_item['retail_price']
        }
    ]
    
    stock_before = frappe.db.get_value("Bin", {"item_code": test_item_code, "warehouse": wh}, "actual_qty") or 0.0
    ...
    try:
        ...
        if res.get("success"):
            ...
            print(f"   Factura: {res['sales_invoice']} | Total: ${res['grand_total']} | Pago: ${res['advance_paid']}")
            
            # Verificar reducción de stock
            stock_after = frappe.db.get_value("Bin", {"item_code": test_item_code, "warehouse": wh}, "actual_qty") or 0.0
            if float(stock_after) == float(stock_before) - 10.0:
                ...
                print(f"⚠️ Alerta: El stock no disminuyó como se esperaba. Antes: {stock_before}, Después: {stock_after}")
        else:
    ...
    try:
        ...
        if sales_order_id in order_names:
            print(f"✅ Éxito: El pedido '{sales_order_id}' figura en la lista de pendientes del administrador.")
        else:
        ...
        stock_before_completion = frappe.db.get_value("Bin", {"item_code": test_item_code, "warehouse": wh}, "actual_qty") or 0.0
        
        comp_res = complete_wholesale_order(
            sales_order_name=sales_order_id,
        ...
        if comp_res.get("success"):
            ...
            print(f"   Factura: {comp_res['sales_invoice']} | Saldo Pendiente: ${comp_res['outstanding_amount']}")
            
            # Verificar reducción de stock
            stock_after_completion = frappe.db.get_value("Bin", {"item_code": test_item_code, "warehouse": wh}, "actual_qty") or 0.0
            if float(stock_after_completion) == float(stock_before_completion) - 20.0:
                print("✅ Éxito: El stock de Distribución disminuyó exactamente 20 unidades tras facturar el pedido.")
            else:
                print(f"❌ Falla: El stock no disminuyó adecuadamente. Antes: {stock_before_completion}, Después: {stock_after_completion}")
        ...
        # 3e. Probar cancelación de pedidos (liberación de stock y borrado)
        ...
        reserved_qty_before = frappe.db.get_value("Bin", {"item_code": test_item_code, "warehouse": wh}, "reserved_qty") or 0.0
        ...
        if cancel_res.get("success"):
            print("✅ Éxito: Pedido cancelado y eliminado con éxito en la base de datos.")
            
            # Verificar que el Sales Order ya no existe
            if not frappe.db.exists("Sales Order", cancel_so_id):
                print("✅ Éxito: El documento Sales Order fue eliminado físicamente del sistema.")
            else:
                print("❌ Falla: El Sales Order aún existe tras la cancelación.")
                
            # Verificar que el Reserved Qty disminuyó exactamente en 15 unidades
            reserved_qty_after = frappe.db.get_value("Bin", {"item_code": test_item_code, "warehouse": wh}, "reserved_qty") or 0.0
            ...
            if float(reserved_qty_after) == float(reserved_qty_before) - 15.0:
                ...
                print(f"❌ Falla: El stock reservado no se liberó correctamente.")
        else:
    ...
    try:
        ...
        print("   Limpiando físicamente reservas antiguas en la base de datos para asegurar disponibilidad...")
        
        # Obtener IDs de pedidos de carritos de eventos
        orders_to_delete = [o.parent for o in frappe.get_all("Sales Order Item", filters={"item_code": "Carrito Paletero"}, fields=["parent"])]
        ...
        if orders_to_delete:
            ...
            if payment_ids:
                frappe.db.sql("DELETE FROM `tabPayment Entry Reference` WHERE parent IN (%s)" % ", ".join(["'%s'" % p for p in payment_ids]))
                frappe.db.sql("DELETE FROM `tabPayment Entry` WHERE name IN (%s)" % ", ".join(["'%s'" % p for p in payment_ids]))
                
            # Eliminar entradas de libro de anticipos (Advance Payment Ledger Entry)
            frappe.db.sql("DELETE FROM `tabAdvance Payment Ledger Entry` WHERE against_voucher_no IN (%s)" % ", ".join(["'%s'" % o for o in orders_to_delete]))
            
            # Eliminar ítems y cabecera del Sales Order
            frappe.db.sql("DELETE FROM `tabSales Order Item` WHERE parent IN (%s)" % ", ".join(["'%s'" % o for o in orders_to_delete]))
            frappe.db.sql("DELETE FROM `tabSales Order` WHERE name IN (%s)" % ", ".join(["'%s'" % o for o in orders_to_delete]))
        ...
        if avail_after.get("available_qty") == initial_available - 1:
            ...
            print(f"❌ Falla: La disponibilidad de carritos no cambió correctamente. Antes: {initial_available}, Después: {avail_after.get('available_qty')}")
        ...
        if not frappe.db.exists("Warehouse", f"Carrito 1 - LP"):
            target_carrito_wh = "Distribucion - LP"
        else:
            target_carrito_wh = "Carrito 1 - LP"
        ...
        se_wh.company = "La Paletixa"
        se_wh.append("items", {
            "item_code": test_item_code,
            "qty": 300.0,
            "t_warehouse": target_carrito_wh,
            "basic_rate": 10.0
        })
        ...
        stock_before_completion = frappe.db.get_value("Bin", {"item_code": test_item_code, "warehouse": target_carrito_wh}, "actual_qty") or 0.0
        ...
        if completion_res.get("success"):
            ...
            print(f"   Factura: {completion_res['sales_invoice']} | Saldo Cobrado: ${completion_res['advance_paid']} | Outstanding: ${completion_res['outstanding_amount']}")
            
            # Verificar reducción de stock en el almacén seleccionado
            stock_after_completion = frappe.db.get_value("Bin", {"item_code": test_item_code, "warehouse": target_carrito_wh}, "actual_qty") or 0.0
            if float(stock_after_completion) == float(stock_before_completion) - 120.0:
                print(f"✅ Éxito: El stock físico en '{target_carrito_wh}' disminuyó exactamente 120 unidades tras facturar el evento.")
            else:
                print(f"❌ Falla: El stock no disminuyó en el almacén elegido. Antes: {stock_before_completion}, Después: {stock_after_completion}")
        else:
        ...
        if cancel_ev_res.get("success"):
            print("✅ Éxito: Reserva cancelada e eliminada físicamente de la base de datos.")
            
            if not frappe.db.exists("Sales Order", cancel_booking_id):
                print("✅ Éxito: El Sales Order de reserva fue eliminado físicamente del sistema.")
            else:
                print("❌ Falla: El Sales Order de la reserva cancelada aún existe en la base de datos.")
            ...
            if avail_after_cancel == avail_before_cancel + 1:
                print("✅ Éxito: La disponibilidad del Carrito Paletero se incrementó correctamente, liberando la fecha.")
            else:
        ...
        if t1 == "+525544332211" and t2 == "+525544332211" and t3 == "+525544332211" and t4 == "+525544332211":
        ...
        if unread_after.get("unread_count") == 1 and len(unread_after.get("notifications")) == 1:
            ...
            print(f"   Título: {notif_item['title']} | Mensaje: {notif_item['message']} | Referencia: {notif_item['reference_name']}")
            ...
            print(f"❌ Falla: El hook no creó la notificación al confirmar el Sales Order. Res: {unread_after}")

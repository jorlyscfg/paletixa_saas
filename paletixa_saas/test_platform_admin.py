...
def run():
    ...
    try:
        ...
        try:
            ...
            print(f"❌ FAILED: Unexpected exception for Guest: {str(e)}")
        ...
        if master_sites:
            ...
            try:
                ...
                if "Este endpoint solo está disponible en el sitio maestro" in str(e):
                    ...
                    print(f"❌ FAILED: Unexpected exception during master site check: {str(e)}")
            finally:
        ...
        try:
            ...
            for t in tenants:
                print(f"   - Tenant: {t['name']} (Company: {t['company_name']}, DB: {t['database_name']}, Active: {t['active']}, Max Branches: {t['max_branches']}, Active Branches: {t['branch_count']})")
        except Exception as e:
        ...
        try:
            ...
            assert bool(master_doc.active) == target_active, "Active status not updated in SaaS Tenant Request"
            assert master_doc.max_branches == target_max_branches, "Max branches not updated in SaaS Tenant Request"
            ...
            if os.path.exists(os.path.join(bench_path, "sites", domain)):
                ...
                assert bool(tenant_config.is_active) == target_active, "Active status not updated in Tenant SaaS Feature Config"
                assert tenant_config.max_branches == target_max_branches, "Max branches not updated in Tenant SaaS Feature Config"
                print("✅ Verified tenant database SaaS Feature Config updated correctly via connection switch")
                ...
                print(f"⚠️ Tenant site folder for '{domain}' does not exist on disk, skipped tenant DB assertion")

            # Restore original values
            print("Restoring original configuration values...")
            update_tenant_config(subdomain=subdomain, active=bool(orig_active), max_branches=orig_max_branches)
            print("✅ Restored original configuration values")
            print("✅ PASSED: Config synchronization working correctly")

        except Exception as e:
            print(f"❌ FAILED: Config synchronization test failed: {str(e)}")
            traceback.print_exc()
        ...
        if test_tenant.get("branches"):
            ...
            try:
                ...
                assert frappe.db.get_value("POS Profile", branch_name, "disabled") == (1 if target_disabled else 0), "POS Profile disabled field mismatch"
                # Verify Warehouse is disabled
                warehouse = frappe.db.get_value("POS Profile", branch_name, "warehouse")
                if warehouse:
                    assert frappe.db.get_value("Warehouse", warehouse, "disabled") == (1 if target_disabled else 0), "Warehouse disabled field mismatch"
                ...
                toggle_tenant_branch(subdomain=subdomain, branch_name=branch_name, disabled=bool(orig_disabled))
                print("✅ Restored original branch state")
                print("✅ PASSED: Branch toggling works")

            except Exception as e:
                print(f"❌ FAILED: Branch toggling test failed: {str(e)}")
                traceback.print_exc()
                try:
                    frappe.destroy()
                    frappe.init(site=original_site, sites_path=os.path.join(get_bench_path(), "sites"))
        ...
        try:
            ...
            try:
                print("Attempting to create a new branch 'Test-Limit-Branch' (should trigger limit enforcement)...")
                ...
                print(f"✅ PASSED: Branch creation blocked with message: {str(e)}")
            except Exception as e:
                print(f"❌ FAILED: Unexpected exception during limit enforcement test: {str(e)}")
                traceback.print_exc()
            ...
            print(f"❌ FAILED: Branch limit enforcement test failed: {str(e)}")
            traceback.print_exc()
            try:
                frappe.destroy()
                frappe.init(site=original_site, sites_path=os.path.join(get_bench_path(), "sites"))
        ...
        try:
            ...
            print(f"Original billing status: exempt={orig_exempt}, last_payment={orig_last_payment}, expiration={orig_expiration}, active={orig_active_state}")
            ...
            update_tenant_config(subdomain=subdomain, active=True, max_branches=orig_max_branches, exempt_from_payment=True)
            assert frappe.db.get_value("SaaS Tenant Request", subdomain, "exempt_from_payment") == 1, "Exempt status not enabled"
            
            print("Testing update_tenant_config with exempt_from_payment=False...")
            update_tenant_config(subdomain=subdomain, active=True, max_branches=orig_max_branches, exempt_from_payment=False)
            assert frappe.db.get_value("SaaS Tenant Request", subdomain, "exempt_from_payment") == 0, "Exempt status not disabled"
            
            # B. Test confirm_tenant_payment updates dates and active flag
            print("Testing confirm_tenant_payment...")
            res = confirm_tenant_payment(subdomain=subdomain)
            ...
            assert updated_doc.last_payment_date is not None, "Payment confirmation did not set last_payment_date"
            ...
            assert frappe.db.get_value("SaaS Tenant Request", subdomain, "active") == 0, "Deactivation sweep did not deactivate past-due tenant"
            ...
            if os.path.exists(os.path.join(bench_path, "sites", domain)):
                ...
                assert frappe.db.get_single_value("SaaS Feature Config", "is_active") == 0, "Deactivation sweep did not sync to Tenant DB"
            ...
            frappe.db.set_value("SaaS Tenant Request", subdomain, {
                "active": 1,
                "exempt_from_payment": 1,
            ...
            assert frappe.db.get_value("SaaS Tenant Request", subdomain, "active") == 1, "Deactivation sweep deactivated an exempt tenant!"
            ...
            print(f"❌ FAILED: Billing deactivation tests failed: {str(e)}")
            traceback.print_exc()
        ...
        print(f"\n❌ CRITICAL EXCEPTION RUNNING TESTS: {str(e)}")
        traceback.print_exc()
